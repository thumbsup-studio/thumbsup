 ...
 *
 * <p>{@code stepOrder}는 {@code quiz.concept.LearnedConceptRecorder}(#233)도 같은 이벤트를
 * 동기 구독해 지식 그래프 학습 기록을 남기는 데 쓴다 — 퀴즈 id 목록처럼 quiz 내부 구현 세부사항까지
 * 이벤트에 싣지 않고, 그 리스너가 필요하면 stepOrder로 직접 조회하게 한다.
 */
public record QuizStepCompletedEvent(Long userId, LocalDate completedOn, int stepOrder) {}
