...
| 분석 범위 | 지정 채널만, 스레드 잠잠(기본 2시간) 후 자동 분석 | 전 채널 감시·실시간 분석 안 함 |
| 일정 관리 범위 | 주간 현황 리포트 + 회의 액션아이템 추적 | 정체 이슈 리마인드·마일스톤 위험 감지는 비범위 |
| 팀원 접근 | Slack 멘션 + MCP stdio 패키지(각자 로컬 실행) | MCP 서버 호스팅 없음 |
| 스택 | TypeScript / Node ≥22, `bridge/`와 동일 계열 | Slack Bolt·better-sqlite3·Octokit·execa |
...
Slack Socket Mode ──▶ 수집기 ──▶ SQLite (messages, threads, action_items, jobs, qa_pending)
                                   │
                  스레드 잠잠 감지 ─▶ 분석기 (claude -p, JSON 스키마 강제)
                                   │
                    ┌──────────────┼───────────────┐
                    ▼              ▼               ▼
...
- **스레드 워처(watcher)**: 스레드의 마지막 메시지 후 2시간(설정값) 경과 시 분석 잡 큐잉. 같은 스레드에 새 메시지가 오면 재분석 대상.
...
- **액션 실행기(actor)**: 분석 결과를 결정적으로 실행 — git 브랜치·커밋·PR(로컬 전용 clone에서 main 최신화 후 작업), GitHub 이슈 생성·보드 필드 배치(Octokit, Projects v2 GraphQL), Slack 게시.
...
- 명세를 바꾸고 싶으면 지정 채널 스레드에서 논의하면 된다 — 봇이 잠잠해진 뒤 PR로 제안한다
