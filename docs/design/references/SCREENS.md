...
**전 화면 Claude 디자인으로 재설계 예정 — 레퍼런스 슬롯은 새 시안이 올 때까지 비워둔다.**
(아래 `기존 레퍼런스`의 6장은 잠정/레거시로, 새 시안이 확정되면 교체된다.)

| 화면 | 라우트(잠정) | 레퍼런스 파일 | 테마 | visual-qa | 상태 |
|---|---|---|---|---|---|
| 홈 / Today | `/` | *(비움 — 재디자인 대기)* | 라이트 | 잠정 `home.png` 연결 유지 | 🟡 재디자인 대기 |
| 퀴즈 | `/quiz` *(잠정)* | *(비움)* | 라이트 | 미연결 | 🟡 재디자인 대기 |
| 해설 | `/answer` *(잠정)* | *(비움)* | 라이트 | 미연결 | 🟡 재디자인 대기 |
| **로그인** | `/login` | *(비움)* | 라이트 | 미연결 | 🟡 신규 시안 대기 |
| **회원가입** | `/signup` | *(비움)* | 라이트 | 미연결 | 🟡 신규 시안 대기 |
| **지식 그래프 / 아틀라스** | `/atlas` | *(비움)* | **다크** | 미연결 | 🟡 신규 시안 대기 |
