...
**Goal:** `apps/web/`에 Next.js+Tailwind+Biome을 스캐폴딩하고, AI 에이전트(사람 보조·봇 모두)가 규약대로 작동하는 기반(CLAUDE.md·스킬·CI·@claude 봇)을 깐다.

**Architecture:** 모노레포에 `apps/web/` 독립 pnpm 패키지를 추가. 에이전트 규약은 CLAUDE.md(+AGENTS.md 심링크) 2개와 레포 스킬로, 품질 게이트는 GitHub Actions CI로, 구현 위임은 `anthropics/claude-code-action`으로 구성.
...
**Files:**
- Create: `apps/web/` (create-next-app 산출물 전체), `.nvmrc`
- Modify: `apps/web/src/app/page.tsx` (플레이스홀더 홈), `apps/web/README.md` (교체), `README.md` (로컬 개발 섹션 추가)

**Interfaces:**
- Produces: `apps/web/package.json`의 `dev`/`build`/`start` 스크립트, `.nvmrc`(내용 `22`) — 이후 모든 태스크·워크플로우가 사용
...
기대: `apps/web/` 생성, `pnpm install` 자동 실행. 기존 git 레포 내부라 git init은 자동 생략됨. 추가 프롬프트가 나오면 기본값 선택.

- [ ] **Step 3: 산출물 확인**

```bash
ls apps/web/src/app/            # page.tsx, layout.tsx, globals.css 존재
grep -c tailwindcss apps/web/package.json   # 1 이상 (v4 의존성)
test ! -e apps/web/eslint.config.mjs && echo "ESLint 없음 OK"
```
...
`apps/web/package.json`에 추가 (기존 키 유지, 최상위에 병합):
...
- [ ] **Step 5: 플레이스홀더 홈으로 교체** (`apps/web/src/app/page.tsx` 전체 교체 — Tailwind 적용 확인용 샘플 페이지, 이슈 acceptance)
...
`apps/web/src/app/layout.tsx`의 metadata를 수정 (title/description만):
...
- [ ] **Step 7: apps/web/README.md 교체** (전체 내용)
...
- Modify: `apps/web/package.json` (devDependency + scripts)
...
```bash
cd apps/web && pnpm add -D -E @biomejs/biome
```

- [ ] **Step 2: `apps/web/biome.json` 생성** (`pnpm biome init` 후 아래로 교체 — `$schema` 버전은 init이 생성한 값 유지)
...
- [ ] **Step 3: package.json scripts 추가** (`apps/web/package.json`의 scripts에 병합 — 기존 dev/build/start 유지)
...
```bash
cd apps/web && pnpm lint:fix && pnpm lint && pnpm typecheck && pnpm build
```
...
```bash
git add apps/web/biome.json apps/web/package.json apps/web/pnpm-lock.yaml apps/web/src
...
**Files:**
- Create: `CLAUDE.md`, `AGENTS.md`(심링크), `apps/web/CLAUDE.md`, `apps/web/AGENTS.md`(심링크)
...
cd apps/web && pnpm typecheck && pnpm lint && pnpm build   # 품질 게이트
​```
...
- [ ] **Step 2: `apps/web/CLAUDE.md` 작성** (전체 내용)
...
cd apps/web && ln -s CLAUDE.md AGENTS.md && cd ..
```
...
cat apps/web/AGENTS.md | head -1    # "# app — Next.js 프론트엔드 규약"
```

- [ ] **Step 5: 커밋**

```bash
git add CLAUDE.md AGENTS.md apps/web/CLAUDE.md apps/web/AGENTS.md
...
**Files:**
- Create: `.claude/skills/verify-apps/web/SKILL.md`
...
---
name: verify-app
description: apps/web/ 코드 변경 후 검증 게이트. app을 수정한 작업을 완료 보고하거나 커밋하기 전 반드시 실행. 사용자가 "검증해", "게이트 돌려"라고 할 때도 트리거.
---
...
```bash
cd apps/web && pnpm typecheck && pnpm lint && pnpm build && echo "verify-app GATE OK"
```
...
**Interfaces:**
- Produces: `apps/web/**`·`server/**` 기준으로 살아있는 CodeRabbit 경로별 리뷰 지침 — Task 9의 PR에서 즉시 검증됨

- [ ] **Step 1: `.coderabbit.yaml` 경로 치환** — `path_instructions`의 `'frontend/**'`→`'apps/web/**'`, `'backend/**'`→`'server/**'`; `path_filters`의 6개 경로 치환:
...
- [ ] **Step 2: README CodeRabbit 문단 수정** — `frontend/**`·`backend/**` 언급을 `apps/web/**`·`server/**`로:

기존: `` `frontend/**`와 `backend/**`에 각기 다른 리뷰 지침을 적용한다 ``
수정: `` `apps/web/**`와 `server/**`에 각기 다른 리뷰 지침을 적용한다 ``
...
- [ ] **Step 3: PR 위에서 자동 검증 확인**
  ...
  - CodeRabbit 리뷰 코멘트가 달리고, `apps/web/**` 파일에 프론트 지침 기반 코멘트인지 확인
