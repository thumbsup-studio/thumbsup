...
**배치 (모노레포):**
- `apps/web/` — 대시보드는 기존 Next.js 앱의 **보호된 라우트 그룹**(예: `apps/web/(authoring)/`)으로 MVP. Vercel·인증 인프라 재사용. (분리 여부는 §12 미해결)
...
1. **대시보드 배치**: 기존 `apps/web/`의 보호된 라우트 그룹(MVP 추천) vs 별도 Next.js 앱. 소비자 학습 앱과 인증 표면·번들을 섞는 것의 트레이드오프.
