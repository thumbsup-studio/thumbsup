...
표기는 세 가지다. `[x]`는 아래 체크리스트를 모두 확인했다는 뜻이고, `[~]`는 일부만 확인했다는 뜻이며, `[ ]`는 확인하지 않았다는 뜻이다. `[~]`가 남아 있는 라우트는 이식 완료로 보지 않는다.
...
Xcode 26.6으로 올린 뒤 iOS 네이티브 dev 빌드(`expo run:ios`)가 처음 가능해져, 아래 세 환경에서 프로덕션 API(`https://thumbsup-api.duckdns.org`)로 라우트를 한 번씩 열었다. 계정은 QA 전용으로 새로 가입한 것을 썼다.

| 환경 | 빌드 | 비고 |
|---|---|---|
| iOS 시뮬레이터 iPhone 17 Pro (iOS 26.5) | `expo run:ios` dev 빌드 | 네이티브 최초 검증 |
| Android 에뮬레이터 `sleepthera_test` | `pnpm dev:install android` | |
| Android 실기기 SM-A336N (Android 16) | 같은 debug APK | 실기기 최초 검증 |

### 이번에 확인한 범위 — `[~]`의 내용

- **화면**: 12개 라우트 중 11개가 정상 데이터로 렌더링되고, 정보 구조·문구·진행 표시·활성 조건이 웹 기준선과 같다. **URL 파라미터 대응 딥링크는 확인하지 못했다** — `thumbsup-dev://history/done?...`을 보냈으나 dev-client가 처리하지 않았다.
- **상태**: 정상 경로와 일부 빈 상태(`/history`의 "아직 배운 개념이 없어요", `/history/graph`의 "아직 연결된 개념이 없어요")만 재현했다. **로딩·오류·재시도·인증 만료·오프라인·15초 타임아웃은 재현하지 않았다.**
- **API·인증**: 각 라우트의 정상 경로 엔드포인트가 실서버 데이터로 동작하는 것을 확인했다(`GET /home`·`/mascot`·`/courses`·브리핑·`/quizzes/next`·힌트·답안 제출·해설·꼬리질문·`/history/graph`·`/auth/me`·`/auth/logout`). SecureStore 세션 복구도 iOS·Android 양쪽에서 확인했다. **토큰 재발급, ADMIN 분기, 중복 탭·취소·늦은 응답 처리는 확인하지 않았다.**
- **접근성·모바일 고유**: 이번 회차 범위 밖이라 그대로 비워 둔다. 다만 실기기에서 하단 탭 라벨이 시스템 내비게이션 바에 가리는 문제를 발견했다(아래).

### `/history/done`을 채우지 못한 이유

복습 마지막 문제(5/5)를 제출하면 해설 화면의 버튼이 `복습 완료`가 아니라 `다음 문제`로 표시되고, 누르면 같은 문제로 되돌아온다. `insight-screen.tsx`의 `context.review.slot >= explanation.totalCount` 판정이 false로 떨어진다. 서버 응답은 정상임을 확인했다(`GET /quizzes/steps/1/{1..5}`가 `slotOrder` 1~5·`totalCount` 5, slot 6은 `QUIZ_NOT_FOUND`). 이 때문에 복습을 끝낼 수 없고 `/history/done`에 도달할 수 없다.

### 함께 발견한 것

- 의견 보내기 시트에서 소프트 키보드가 `보내기` 버튼을 완전히 가린다(iOS). 모바일 고유 체크리스트의 키보드 항목에 해당한다.
- 지식그래프 WebView의 초기 뷰포트가 노드를 화면에 맞추지 않아 일부만 보인다(iOS·Android 공통).
- 하단 탭 라벨이 실기기의 시스템 내비게이션 바에 가려 잘린다. `(tabs)/_layout.tsx`의 `tabBarStyle`이 safe area inset을 반영하지 않는다. 에뮬레이터에서는 드러나지 않았다.
- 회전은 `app.config.ts`가 `orientation: "portrait"`이라 세로 고정이 유지된다. 모바일 고유 체크리스트의 회전 항목은 "가로 지원"이 아니라 "세로 고정 유지"로 판정해야 한다.
