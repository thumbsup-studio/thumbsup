...
Vercel에 GitHub Actions로 배포한다(사용자 앱 `web-deploy.yml`, 저작 앱 `authoring-deploy.yml`, Git 연동 아님).

- **main 머지** → 변경된 Next 앱의 프로덕션 자동 배포
- **PR (`apps/web/**` 또는 `apps/authoring/**` 변경)** → 해당 앱 프리뷰 배포 + PR에 프리뷰 URL 코멘트 자동 게시
...
| `VERCEL_ORG_ID` | GitHub Secrets | 두 Vercel 프로젝트의 조직 식별 |
| `VERCEL_PROJECT_ID_WEB` | GitHub Secrets | 사용자 앱 프로젝트 식별. 미설정 시 기존 `VERCEL_PROJECT_ID` 사용 |
| `VERCEL_PROJECT_ID_AUTHORING` | GitHub Secrets | 저작 앱 프로젝트 식별 |
...
| `NEXT_PUBLIC_AUTHORING_URL` | 사용자 웹 Vercel 환경변수 | ADMIN 로그인과 기존 `/authoring/*` 경로가 이동할 저작 앱 URL |
...
저작 앱 최초 설정과 운영 CORS·롤백 절차는 [`apps/authoring/docs/deploy.md`](apps/authoring/docs/deploy.md)를 참고한다.
