---
name: verify-app
description: apps/web·apps/authoring·apps/mobile 변경 후 실행하는 품질 게이트. 앱 작업을 완료 보고하거나 커밋하기 전에 사용하며, 사용자가 "검증해", "게이트 돌려", "앱 검사해", "모바일 테스트 돌려", "저작 앱 검사해", "PR 전에 확인해"라고 할 때도 트리거.
---

# verify-app — 앱 품질 게이트

변경한 앱에 해당하는 게이트를 실행한다. 여러 앱을 건드렸으면 각각 실행한다.
...
`apps/authoring/` 디렉터리에서 순서대로 실행. **하나라도 실패하면 수정 후 1번부터 재실행.**
...
pnpm lint         # 2. 실패 시 pnpm lint:fix 후 재확인 (수정 diff 검토 필수)
pnpm build        # 3. 프로덕션 빌드 성공
pnpm test         # 4. Vitest 통과
pnpm check:design # 5. 토큰 규칙 위반 0건
```
