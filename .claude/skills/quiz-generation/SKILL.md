...
엘리스 GPT-5.4(`elice-models` 스킬 참조)로 주제 하나당 5문제(스텝 1개)를 생성해 DB에 저장한다. 상시 HTTP 엔드포인트가 아니라 **CLI 전용**이다 — LLM 호출 비용이 공용 API 키로 나가기 때문.

> **이제 저작 파이프라인(#174·#175·#176)과 공존한다.** 팀원 개인 AI CLI 구독으로 웹 대시보드에서 생성·검수·승인하는 경로는 **`authoring` 스킬**을 볼 것(ADMIN 게이트 뒤에서 HTTP로 열려 있다).
> 이 문서의 **프롬프트 조립(`QuizGenerationPromptBuilder`)과 검증 규칙(마커·빈칸·슬롯 구성)은 두 파이프라인이 그대로 공유**한다 — 프롬프트나 검증기를 고치면 양쪽에 다 영향이 간다.
...
- `--level=basic|standard|advanced`(대소문자 무관, 기본 standard)로 **콘텐츠 난이도**를 조절할 수 있다 — 아래 "스텝 구성"의 유형별 난이도(하2·중2·상1)와는 별개 축이다. 한 번의 실행에 포함된 모든 스텝에 동일하게 적용된다. 예: `--topic="CPU 스케줄링 심화" --level=advanced`.
- `--courseId=<id>`로 이 스텝이 속할 코스를 지정한다. 생략하면 가장 먼저 생성된 코스(운영체제)로 계속 생성된다 — 기존 사용법 그대로 안전. 다른 커리큘럼(예: 디자인패턴)을 생성할 땐 그 코스의 id를 지정해야, `GET /api/v1/quizzes/next`가 그 코스에서만 이 스텝을 보여준다(#117 코스별 독립 진행 상태).
...
**이 표의 "난이도"는 문제 형식(유형) 난이도이지, 콘텐츠 난이도가 아니다.** 슬롯 순서·유형·난이도 조합은 절대 바뀌지 않는다(위에서 서술한 대로 고정, 변경 불가) — 늘 OX 2개·사지선다 2개·키워드 빈칸 1개다. 그 안에서 "얼마나 쉬운/어려운 개념을 다루는가"는 별도 축이며, `--level`(CLI) → `GenerationLevel`(BASIC/STANDARD/ADVANCED) → `QuizGenerationPromptBuilder`의 유형별 콘텐츠 난이도 힌트로 조절한다. 기본값 STANDARD는 힌트 없이 모델 판단에 맡긴다(기존 동작과 동일).
