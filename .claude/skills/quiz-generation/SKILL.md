...
- 꼬리질문의 "첫 등장 1회"는 **필드 단위** 규칙이다. `blocks`는 블록 하나가 곧 한 필드이므로, 서로 다른 블록에서 같은 키워드를 한 번씩 마킹하는 것은 허용된다. 블록들을 이어붙여 한 번에 검사하면 규칙의 의미가 달라진다.
...
2. **사람이 전 문제를 검수** — 정답·해설·키워드 커버리지·문제 해설 3개 컬럼 전체의 키워드별 마커 1회·꼬리질문까지 전부 눈으로 확인. LLM 생성물이라 신뢰하지 않는다.
