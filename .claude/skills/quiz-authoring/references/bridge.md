...
브리지는 시작할 때 저작 앱 주소를 출력한다. 기본값은 `https://authoring-thumbsup.vercel.app`이고, 프리뷰나 롤백 배포를 쓸 때는 `THUMBSUP_AUTHORING_URL`로 덮어쓴다. API 접속 주소는 별도로 `~/.thumbsup/bridge.json`의 `serverUrl`을 쓴다.
