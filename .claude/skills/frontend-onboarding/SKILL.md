...
description: 백엔드 개발자가 apps/web/(Next.js) 프론트 작업을 맡을 때의 온보딩 지도. 환경 준비물·개발 서버, 프론트 코드 3층 구조(Spring 대응), 작업 순서, 단계별 필수 스킬 라우팅을 안내하며 apps/mobile 작업은 mobile-onboarding으로 연결한다. 규칙 정본은 각 스킬·문서 링크로 참조. 사용자가 "프론트 처음인데", "web 작업 뭐부터 해", "화면 어떻게 만들어", "프론트 환경 세팅", "서버만 하다가 프론트 맡았어"라고 할 때 트리거.
---
...
> 작업 대상이 `apps/mobile/`이면 이 웹 온보딩 절차는 적용되지 않는다. 대신 `mobile-onboarding` 스킬을 읽는다 — 네이티브 툴체인 준비, 앱 실행 방식 세 가지, 모바일 코드 구조가 웹과 다르다.
