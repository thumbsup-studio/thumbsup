...
function getSessionStorage(): StorageLike | undefined {
  const storage = (globalThis as { sessionStorage?: Partial<StorageLike> }).sessionStorage;
  return typeof storage?.getItem === "function" && typeof storage.setItem === "function"
    ? (storage as StorageLike)
    : undefined;
}

export function hasPlayedCompletionFanfare(
  key: string,
  storage: StorageLike | undefined = getSessionStorage(),
): boolean {
  try {
    return storage?.getItem(fanfareKey(key)) !== null && storage !== undefined;
  } catch {
    return false;
  }
}

export function markCompletionFanfarePlayed(
  key: string,
  storage: StorageLike | undefined = getSessionStorage(),
) {
  try {
    storage?.setItem(fanfareKey(key), "1");
  } catch {
    /* 위 주석 참고 */
  }
}
