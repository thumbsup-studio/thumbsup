...
- **승인은 즉시 라이브 반영이고 되돌리는 API가 없다.** 로컬 개발 시 `.env.local`에 `NEXT_PUBLIC_API_URL=http://localhost:8080`을 반드시 넣는다. 미설정 시 기본값이 운영 API라, 로컬 화면으로 운영 문제를 승인하게 된다.
...
- 화면마다 2차 방어를 둔다. `ApiError.status`가 401이면 `/login`, 403이면 목록으로 돌린다. 세션 중 권한이 바뀌는 경우를 위한 것이므로 새 화면에서도 빼지 않는다.
...
`@thumbsup/core`가 `package.json`에 선언돼 있지만 소스 어디에서도 쓰지 않는다. 저작 앱에 공용 로직이 필요해지면 그때 쓰고, 끝까지 안 쓰면 의존성에서 걷어낸다.
